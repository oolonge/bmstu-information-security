...
void ConsoleInterface::run() {
    std::cout << "=== DES Шифрование - Лабораторная работа 2.2 ===\n\n";
...
void ConsoleInterface::handle_load_key() {
    ...
    des.load_key(key_path);
    
    std::cout << "\n";
}

void ConsoleInterface::handle_save_key() {
    if (!des.is_initialized()) {
    ...
    if (key_file.empty()) key_file = "des_key";
    
    std::string key_path = "out/" + key_file + ".key";
    
    des.save_key(key_path);
...
void ConsoleInterface::handle_encrypt_file() {
    ...
    // Получаем список всех файлов (кроме .des и .key)
    ...
    std::string output_file = selected_file + ".des";
    std::string output_path = "out/" + output_file;
    
    std::cout << "\nШифрование: " << selected_file << " -> " << output_file << "\n";
    
    des.encrypt_file(input_path, output_path);
...
void ConsoleInterface::handle_decrypt_file() {
    ...
    std::vector<std::string> des_files = get_files_in_out_directory(".des");
    
    if (des_files.empty()) {
        std::cout << "В директории out/ не найдено зашифрованных файлов (.des).\n\n";
        return;
    }
    
    std::string selected_file = select_file_from_list(des_files, "Выберите файл для расшифровки");
    ...
    // Удаляем расширение .des из имени
    std::string output_file = selected_file;
    if (output_file.size() > 4 && output_file.substr(output_file.size() - 4) == ".des") {
    ...
    des.decrypt_file(input_path, output_path);
